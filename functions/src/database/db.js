...
const { Pool, Query } = require('pg'); // pg = postgres
const dayjs = require('dayjs'); // 날짜 찍어주는 놈
const dotenv = require('dotenv');

dotenv.config();

// DB Config (유저, 호스트, DB 이름, 패스워드)를 로딩해줍시다. => DB랑 연결~~
const dbConfig = require('../config/dbConfig');

// NODE_ENV라는 글로벌 환경변수(node에서 제공하는 변수)를 사용해서, 현재 환경이 어떤 '모드'인지 판별해줍시다.
...
// pg 라이브러리 내부의 함수를 살짝 손봐서 SQL 쿼리문이 콘솔에 찍히게 만들어 줌
const submit = Query.prototype.submit; // submit은 Query 내부의 함수임~~!
...
// 커넥션 풀을 생성해줍니다.
const pool = new Pool({
  ...dbConfig,  // 연결한 DB를 pool에다가 퐁당~
  connectionTimeoutMillis: 60 * 1000,
  idleTimeoutMillis: 60 * 1000,
});

// 위에서 생성한 커넥션 풀에서 커넥션을 빌려오는 함수를 재정의!!
// 기본적으로 제공되는 pool.connect()와 pool.connect().release() 함수에 디버깅용 메시지를 추가하는 작업!!!
const connect = async (req) => { // req를 받을 수 있도록 손좀 봐주자
  const now = dayjs();
  const string =
    !!req && !!req.method
